...
onMounted(async () => {
  try {
    ...
    if (settings) {
      config.value.officeLat = settings.office_lat ?? config.value.officeLat;
      config.value.officeLng = settings.office_lng ?? config.value.officeLng;
      config.value.officeRadius = settings.geofence_radius ?? config.value.officeRadius;
    }
  } catch (err) {
...
async function saveSettings() {
  ...
  try {
    await attendanceApi.updateSettings({
      office_lat: config.value.officeLat,
      office_lng: config.value.officeLng,
      geofence_radius: config.value.officeRadius,
    } as Parameters<typeof attendanceApi.updateSettings>[0]);
