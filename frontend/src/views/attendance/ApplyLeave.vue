...
const student = ref<{ id: string; name: string } | null>(null);
...
async function submitApplication() {
  ...
  try {
    ...
    await attendanceApi.submitLeave({
      leave_type: form.value.type,
      start_date: form.value.startDate,
      end_date: form.value.endDate,
      reason: form.value.reason + (attachmentUrl ? ` [attachment: ${attachmentUrl}]` : ""),
    });

    showSuccess.value = true;
  } catch (err: unknown) {
    console.error("Submission failed:", err);
    alert("Failed to submit application: " + (err instanceof Error ? err.message : err));
  } finally {
